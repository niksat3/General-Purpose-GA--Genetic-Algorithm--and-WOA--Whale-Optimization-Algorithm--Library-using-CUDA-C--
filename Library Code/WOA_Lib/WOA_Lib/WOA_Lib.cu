...
/*
class coba2 : public WOA_Lib<12> {

public:
	float* coord;
	int sizeAgent;

	coba2() : WOA_Lib((long)100, 20) {
		sizeAgent = 12;
		run();
		float* leader = getLeader();
		printf("City Index: %f",leader[0]);
		for (int i = 1; i < sizeAgent; i++) {
			printf(",%f",leader[i]);
		}
	};

	void doInitialization() {
		coord = new float[pow(sizeAgent/2,2)];
		float* newAgent = this->getSearchAgent();
		
		float coordTemp[] = {
			100.0f, 100.0f, 100.0f,   9.0f,   3.0f, 100.0f,
			100.0f, 100.0f, 100.0f, 100.0f, 100.0f,   5.0f,
			100.0f, 100.0f, 100.0f, 100.0f, 100.0f,  10.0f,
			  9.0f, 100.0f, 100.0f, 100.0f,   8.0f,   6.0f,
			  4.0f, 100.0f, 100.0f,   8.0f, 100.0f, 100.0f,
			100.0f,   5.0f,  10.0f,   6.0f, 100.0f, 100.0f
		};
		for (int j = 0; j < 36; j++) {
			coord[j] = coordTemp[j];
		}
		...
		for (int i = 0; i < getNumberOfSearchAgent(); i++) {
			float* a = new float[sizeAgent];
			randomSearchAgent(a);
			for (int j = 0; j < sizeAgent; j++) {
				newAgent[(i * sizeAgent) + j] = a[j];
			}
		}
		setSearchAgent(newAgent);
	};

	void doFitnessCheck(long searchAgentAmount) {
		float* searchAgentTemp = this->getSearchAgent();
		float* fitnessTemp = this->getFitness();
		bool doneVisited[] = {false,false,false,false,false,false};
		for (int i = 0; i < searchAgentAmount; i++)
		{
			float nilai = 0.0f;
			int i_d = i;
			bool cek = true;
			for (int j = 0; j < sizeAgent-1; j++) {
				cek = true;
				int indexCity1 = abs((int)searchAgentTemp[i_d*sizeAgent+j+1])%6;
				int indexCity2 = abs((int)searchAgentTemp[i_d*sizeAgent+j])%6;
				float dist = coord[indexCity1*6+indexCity2];
				nilai += dist;
				doneVisited[indexCity1] = true;
				for (int j = 0; j < 6; j++) {
					if(!doneVisited[j]) cek = false;
				}
				if(cek) break;
				cek = false;
			}
			if(!cek) {
				int indexCity1 = abs((int)searchAgentTemp[i_d*sizeAgent])%6;
				int indexCity2 = abs((int)searchAgentTemp[i_d*sizeAgent+11])%6;
				int dist = coord[indexCity1*6+indexCity2];
				nilai += dist;
			}
			fitnessTemp[i_d] = 1200 - nilai;
		}
	};

	void randomSearchAgent(float* newSearchAgent) {
		for (int i = 0; i < sizeAgent; i++) {
			newSearchAgent[i] = (float)(rand() / (float)RAND_MAX)*5;
		}
	};
